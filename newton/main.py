        ...
        if not viewer.is_paused():
            with wp.ScopedTimer("step", active=False):
                for _ in range(substeps):
                    ...
                    eef_world_right = wp.transform(*state_0.body_q.numpy()[5])
                    eef_world_left = wp.transform(*state_0.body_q.numpy()[13])

                    print(f"body_q_len: {len(state_0.body_q.numpy())}")
                    ...
                    print(f"pos_right: {pos_right}")
                    print(f"rot_right: {rot_right}")
                    ...
                    print(f"pos_left: {pos_left}")
                    print(f"rot_left: {rot_left}")
                    ...
                    print(f"target_pos_right: {target_pos_right}")
                    # print(f"target_ori_right: {target_ori_right}")
                    ...
                    print(f"target_pos_left: {target_pos_left}")
                    # print(f"target_ori_left: {target_ori_left}")
                    ...
                    # print(f"joint_angles_right: {joint_angles_right}")
                    # print(f"joint_angles_left: {joint_angles_left}")

                    # set the joint targets
                    control.joint_target.assign(
                        wp.array(
                            joint_angles_right.tolist() + joint_angles_left.tolist(),
                            dtype=wp.float32,
                        )
                        # wp.array([0.0] * 18, dtype=wp.float32)
                    )
